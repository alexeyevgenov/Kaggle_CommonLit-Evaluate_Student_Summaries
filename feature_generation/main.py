...
def data_preparation() -> None:
    ...
    for i in range(CONFIG.num_folds):
        ...
        if TEST_MODE:
            input_df = pd.read_feather(path=CONFIG.init_data_storage + f"/fold {i}.ftr")[:51]
        else:
        ...
        df_processed.reset_index(drop=True).to_feather(CONFIG.storage + "/" + CONFIG.version + f"/preprocessed fold {i}.ftr")
